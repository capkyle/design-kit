...
export type DesignKitValues<T> = T;
...
): DesignKitValues<ResolvedValues<T>> {
  const panelId = `${name}-${++designKitInstance}`;
  const resolve = () => buildResolvedValues(config, DesignKitStore.getValues(panelId), '') as ResolvedValues<T>;
